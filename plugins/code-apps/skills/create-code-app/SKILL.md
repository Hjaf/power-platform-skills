...
**Load EDS fonts** — EDS 2.0 components require both the **Inter** (UI/body) and **Equinor** (headings) font families. Add the appropriate CDN stylesheet to `index.html` `<head>`. Refer to the [EDS typography documentation](https://eds.equinor.com/docs/Next/foundation/design-tokens/typography#loading-fonts) for the latest guidance on which stylesheet(s) to use. At time of writing, the recommended option is:
...
> **Note:** The older `equinor-font.css` only includes the Equinor font. Using it alone with EDS 2.0 components causes UI elements (Button, TextField, etc.) to fall back to a generic sans-serif because Inter is missing.

> **CSP Notice:** After the first deploy, inform the user that the target environment's Content Security Policy must allow loading external resources from the EDS CDN. Both `style-src` and `font-src` directives need `https://cdn.eds.equinor.com` added via the Power Platform Admin Center (Environments → Settings → Product → Privacy + Security → Content security policy → App tab). Without this, the font stylesheet will be blocked and text falls back to system fonts. See [troubleshooting.md](./references/troubleshooting.md#eds-font-loading-csp) for details.
